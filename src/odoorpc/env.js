...
export class Environment {
  ...
  action_info_get(action_id) {
    const action = new service.Action(action_id)
    return action.info
  }

  treeview(action_id) {
    const view = new service.TreeView(action_id, { env: this })
    return view
  }

  formview(action_id) {
    const view = new service.FormView(action_id, { env: this })
    return view
  }

  x2mtree(field_info) {
    const view = new service.X2mTree(field_info, { env: this })
    return view
  }

  x2mform(field_info) {
    const view = new service.X2mForm(field_info, { env: this })
    return view
  }

  relation(field_info, payload = {}) {
    const view = new service.Relation(field_info, { env: this, ...payload })
    return view
  }
