...
export default {
  ...
  watch: {
    value_readonly: {
      handler: function (newVal = [], oldval = []) {
        if (!this.check_array_equ(newVal, oldval)) {
          console.log('m2m Tags, watch', this.fieldInfo, newVal, oldval)
  ...
  methods: {
    ...
    async load_m2m_tags_data(ids) {
      console.log('m2m Tags, load_m2m_tags_data', this.fieldInfo, ids)
